...
BEGIN
	...
	ELSEIF  nie is not null and comproba_pacient('nie',nie) = '0' THEN
		RETURN '5';
	ELSEIF passaporte is not null and comproba_pacient('passaporte',passaporte) = '0' THEN
		RETURN '6';
	END IF;
	*/
	data_naix_date := cast (data_naix as date);
	/* to_date(var,'DD-MM-YYYY')*/
	/*(nom,cognom,dni,data_naix,sexe,adreca,ciutat,c_postal,telefom,email,num_ss,num_cat,nif,passaporte)*/
	--raise notice '% % % % % % % % % % % % % %',nom,cognom,dni,data_naix,sexe,adreca,ciutat,c_postal,telefom,email,num_ss,num_cat,nif,passaporte;
